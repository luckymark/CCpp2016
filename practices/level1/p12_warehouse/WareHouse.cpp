...
void readFromTxt()
{
	...
	int num;
	while (fscanf_s(stream, "%s", cargo->type,128)!=EOF)   //从文件读入
	{
...
void init(int v_operand)
{
	...
	case EXPORT:
		...
		break;
	case EXIT:
		freopen_s(&stream,"WareHouse.txt", "w", stderr);   //写入文件
...
bool check(char *t1, char *t2,int len1,int len2)   //比较是否是要寻找的货物
{
	int tot = 0;
